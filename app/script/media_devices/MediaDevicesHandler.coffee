...
class z.media_devices.MediaDevicesHandler
  ###
  ...
  ###
  constructor: (@media_devices_repository) ->
    @logger = new z.util.Logger 'z.media_devices.MediaDevicesHandler', z.config.LOGGER.OPTIONS

    @media_stream_handler = @media_devices_repository.media_stream_handler
  ...
  initialize_media_devices: =>
    return if not z.media_devices.MediaDevicesRepository.supports_media_devices()
  ...
  _set_current_devices: =>
    @current_device_id.audio_input z.util.StorageUtil.get_value(z.media_devices.MediaDeviceType.AUDIO_INPUT) or 'default'
    @current_device_id.audio_output z.util.StorageUtil.get_value(z.media_devices.MediaDeviceType.AUDIO_OUTPUT) or 'default'
    @current_device_id.video_input z.util.StorageUtil.get_value z.media_devices.MediaDeviceType.VIDEO_INPUT
  ...
  _subscribe_to_observables: =>
    @available_devices.audio_input.subscribe (media_devices) =>
      @_update_current_index_from_devices z.media_devices.MediaDeviceType.AUDIO_INPUT, media_devices if media_devices

    @available_devices.audio_output.subscribe (media_devices) =>
      @_update_current_index_from_devices z.media_devices.MediaDeviceType.AUDIO_OUTPUT, media_devices if media_devices

    @available_devices.screen_input.subscribe (media_devices) =>
      @_update_current_index_from_devices z.media_devices.MediaDeviceType.SCREEN_INPUT, media_devices if media_devices

    @available_devices.video_input.subscribe (media_devices) =>
      @_update_current_index_from_devices z.media_devices.MediaDeviceType.VIDEO_INPUT, media_devices if media_devices

    @current_device_id.audio_input.subscribe (media_device_id) =>
      z.util.StorageUtil.set_value z.media_devices.MediaDeviceType.AUDIO_INPUT, media_device_id
      if media_device_id and @media_stream_handler.local_media_streams.audio()
        @media_stream_handler.replace_input_source z.media_devices.MediaType.AUDIO
        @_update_current_index_from_id z.media_devices.MediaDeviceType.AUDIO_INPUT, media_device_id

    @current_device_id.audio_output.subscribe (media_device_id) =>
      z.util.StorageUtil.set_value z.media_devices.MediaDeviceType.AUDIO_OUTPUT, media_device_id
      if media_device_id
        @media_element_handler.switch_media_element_output media_device_id
        @_update_current_index_from_id z.media_devices.MediaDeviceType.AUDIO_OUTPUT, media_device_id

    @current_device_id.screen_input.subscribe (media_device_id) =>
      if media_device_id and @media_stream_handler.local_media_streams.video() and @media_stream_handler.local_media_type() is z.media_devices.MediaType.SCREEN
        @MediaDevicesRepository.coffee.media_stream_handler.replace_input_source z.media_devices.MediaType.SCREEN
        @_update_current_index_from_id z.media_devices.MediaDeviceType.SCREEN_INPUT, media_device_id

    @current_device_id.video_input.subscribe (media_device_id) =>
      z.util.StorageUtil.set_value z.media_devices.MediaDeviceType.VIDEO_INPUT, media_device_id
      if media_device_id and @media_stream_handler.local_media_streams.video() and @media_stream_handler.local_media_type() is z.media_devices.MediaType.VIDEO
        @media_stream_handler.replace_input_source z.media_devices.MediaType.VIDEO
        @_update_current_index_from_id z.media_devices.MediaDeviceType.VIDEO_INPUT, media_device_id
      ...
      if media_devices
        ...
        for media_device in media_devices
          switch media_device.kind
            when z.media_devices.MediaDeviceType.AUDIO_INPUT
              @available_devices.audio_input.push media_device
            when z.media_devices.MediaDeviceType.AUDIO_OUTPUT
              @available_devices.audio_output.push media_device
            when z.media_devices.MediaDeviceType.VIDEO_INPUT
        ...
        throw new z.calling.CallError z.media_devices.MediaDevicesError::TYPE.NO_DEVICES_FOUND
    .catch (error) =>
  ...
  update_current_devices: (is_videod) =>
    ...
    .then =>
      ...
      _check_device z.media_devices.MediaType.AUDIO, z.media_devices.MediaDeviceType.AUDIO_INPUT
      _check_device z.media_devices.MediaType.VIDEO, z.media_devices.MediaDeviceType.VIDEO_INPUT if is_videod
