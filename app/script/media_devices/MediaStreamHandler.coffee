...
class z.calling.handler.MediaStreamHandler
  ...
  @detect_media_stream_type: (media_stream) ->
    if media_stream.getVideoTracks()?.length
      if media_stream.getAudioTracks()?.length
        media_stream.type = z.media_devices.MediaType.AUDIO_VIDEO
      else
        media_stream.type = z.media_devices.MediaType.VIDEO
    else if media_stream.getAudioTracks()?.length
      media_stream.type = z.media_devices.MediaType.AUDIO
    else
      media_stream.type = z.media_devices.MediaType.NONE
    return media_stream
  ...
  @param media_type [z.media_devices.MediaType.AUDIO_VIDEO]
  @return [Array] Array of MediaStreamTracks optionally matching the requested type
  ###
  @get_media_tracks: (media_stream, media_type = z.media_devices.MediaType.AUDIO_VIDEO) ->
    switch media_type
      when z.media_devices.MediaType.AUDIO
        media_stream_tracks = media_stream.getAudioTracks()
      when z.media_devices.MediaType.AUDIO_VIDEO
        media_stream_tracks = media_stream.getTracks()
      when z.media_devices.MediaType.VIDEO
  ...
  constructor: (@call_center) ->
    ...
    @local_media_type = ko.observable z.media_devices.MediaType.AUDIO
  ...
  get_media_stream_constraints: (request_audio = false, request_video = false) ->
    ...
    .then =>
      ...
      media_type = if request_video then z.media_devices.MediaType.VIDEO else z.media_devices.MediaType.AUDIO
  ...
  get_screen_stream_constraints: =>
    return new Promise (resolve, reject) =>
      if window.desktopCapturer
        ...
        resolve [z.media_devices.MediaType.SCREEN, constraints]
  ...
  initiate_media_stream: (conversation_id, is_videod) =>
    ...
    .then (media_stream_info) =>
      ...
      @local_media_type z.media_devices.MediaType.VIDEO if is_videod
  ...
  replace_media_stream: (media_stream_info) =>
    ...
    update_promise.then (resolve_array) =>
      ...
      if media_stream_info.type is z.media_devices.MediaType.AUDIO
        @_release_media_stream @local_media_streams.audio(), z.media_devices.MediaType.AUDIO
      else
        @_release_media_stream @local_media_streams.video(), z.media_devices.MediaType.VIDEO
      @set_local_media_stream media_stream_info

  ###
  Update the used MediaStream after a new input device was selected.
  @param type [z.media_devices.MediaType] Media type of device that was replaced
  ###
  replace_input_source: (media_type) =>
    switch media_type
      when z.media_devices.MediaType.AUDIO
        constraints_promise = @get_media_stream_constraints true, false
      when z.media_devices.MediaType.SCREEN
  ...
  @param media_type [z.media_devices.MediaType] Type of MediaStream to be requested
  ...
  request_media_stream: (media_type, media_stream_constraints, conversation_id) =>
    return new Promise (resolve, reject) =>
      if not @call_center.media_devices_handler.has_microphone()
        ...
        @_show_device_not_found_hint z.media_devices.MediaType.AUDIO, conversation_id
        reject new z.media_devices.MediaDevicesError z.media_devices.MediaDevicesError::TYPE.NO_MICROPHONE_FOUND
      else if not @call_center.media_devices_handler.has_camera() and media_type is z.media_devices.MediaType.VIDEO
        @logger.log @logger.levels.WARN, "Requesting MediaStream access aborted - 'No camera'"
        @_show_device_not_found_hint z.media_devices.MediaType.VIDEO, conversation_id
        reject new z.media_devices.MediaDevicesError z.media_devices.MediaDevicesError::TYPE.NO_CAMERA_FOUND
      else
  ...
  ###
  set_local_media_stream: (media_stream_info) =>
    if media_stream_info.type in [z.media_devices.MediaType.AUDIO, z.media_devices.MediaType.AUDIO_VIDEO]
      @local_media_streams.audio media_stream_info.stream
    if media_stream_info.type in [z.media_devices.MediaType.AUDIO_VIDEO, z.media_devices.MediaType.VIDEO]
      @local_media_streams.video media_stream_info.stream

  ###
  Clear the permission request hint timeout or hide the warning.
  @private
  @param media_type [z.media_devices.MediaType] Type of requested stream
  ###
  _clear_permission_request_hint: (media_type) ->
  ...
  @param media_type [z.media_devices.MediaType] Type of requested MediaStream
  @param conversation_id [String] Conversation ID
  ###
  _initiate_media_stream_failure: (error, media_type, conversation_id) =>
  ...
  @param media_type [z.media_devices.MediaType] Type of MediaStreamTracks to be released
  ###
  _release_media_stream: (media_stream, media_type = z.calling.media_devices.AUDIO_VIDEO) =>
  ...
  @param media_type [z.media_devices.MediaType] Type of failed request
  ###
  _replace_input_source_failure: (error, media_type) ->
    if media_type is z.media_devices.MediaType.SCREEN
  ...
  @private
  @param media_type [z.media_devices.MediaType] Type of device not found
  @param conversation_id [String] Optional conversation ID
  ###
  _show_device_not_found_hint: (media_type, conversation_id) ->
    if media_type is z.media_devices.MediaType.AUDIO
      amplify.publish z.event.WebApp.WARNING.SHOW, z.ViewModel.WarningType.NOT_FOUND_MICROPHONE
    else if media_type is z.media_devices.MediaType.VIDEO
  ...
  @private
  @param media_type [z.media_devices.MediaType] Type of media access request
  ###
  _show_permission_denied_hint: (media_type) ->
    switch media_type
      when z.media_devices.MediaType.AUDIO
  ...
  add_remote_media_stream: (media_stream_info) =>
    switch media_stream_info.type
      ...
      when z.media_devices.MediaType.AUDIO_VIDEO, z.media_devices.MediaType.VIDEO
  ...
  toggle_camera_paused: =>
    if @local_media_streams.video() and @local_media_type() is z.media_devices.MediaType.VIDEO
      @_toggle_video_enabled()
    else
      @replace_input_source z.media_devices.MediaType.VIDEO
  ...
  toggle_screen_shared: =>
    if @local_media_streams.video() and @local_media_type() is z.media_devices.MediaType.SCREEN
      @_toggle_screen_enabled()
    else
      @replace_input_source z.media_devices.MediaType.SCREEN
  ...
  reset_self_states: =>
    ...
    @local_media_type z.media_devices.MediaType.AUDIO
  ...
  @param media_type [z.media_devices.MediaType] Type of state to enable
  ###
  _set_self_stream_state: (media_type) ->
    switch media_type
      when z.media_devices.MediaType.AUDIO
      ...
      when z.media_devices.MediaType.SCREEN
        ...
        @local_media_type z.media_devices.MediaType.SCREEN
      when z.media_devices.MediaType.VIDEO
        @self_stream_state.videod true
        @self_stream_state.screen_shared false
        @local_media_type z.media_devices.MediaType.VIDEO
  ...
  _set_stream_state: (media_stream_info) ->
    if media_stream_info.type in [z.media_devices.MediaType.AUDIO, z.media_devices.MediaType.AUDIO_VIDEO]
      audio_stream_tracks = z.calling.handler.MediaStreamHandler.get_media_tracks media_stream_info.stream, z.media_devices.MediaType.AUDIO
      audio_stream_tracks[0].enabled = not @self_stream_state.muted()

    if media_stream_info.type in [z.media_devices.MediaType.AUDIO_VIDEO, z.media_devices.MediaType.VIDEO]
      video_stream_tracks = z.calling.handler.MediaStreamHandler.get_media_tracks media_stream_info.stream, z.media_devices.MediaType.VIDEO
  ...
  ###
  _toggle_audio_enabled: ->
    @_toggle_stream_enabled z.media_devices.MediaType.AUDIO, @local_media_streams.audio(), @self_stream_state.muted
    .then (audio_track) =>
  ...
  ###
  _toggle_screen_enabled: ->
    @_toggle_stream_enabled z.media_devices.MediaType.VIDEO, @local_media_streams.video(), @self_stream_state.screen_shared
    .then (video_track) =>
  ...
  ###
  _toggle_video_enabled: ->
    @_toggle_stream_enabled z.media_devices.MediaType.VIDEO, @local_media_streams.video(), @self_stream_state.videod
    .then (video_track) =>
  ...
  @private
  @param media_type [z.media_devices.MediaType] Media type to toggle
