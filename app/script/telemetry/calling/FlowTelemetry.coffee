...
class z.telemetry.calling.FlowTelemetry
  ...
  schedule_check: (timeout) ->
    window.setTimeout =>
      @check_stream z.media_devices.AUDIO, timeout
      @check_stream z.media_devices.VIDEO, timeout if @call_et.is_remote_screen_shared() or @call_et.is_remote_videod()
    , timeout
  ...
  _update_from_inbound_rtp: (report, stats) ->
    if report.mediaType in [z.media_devices.MediaType.AUDIO, z.media_devices.MediaType.VIDEO]
    ...
    if statistics
      ...
      for key, value of statistics
        if _.isObject value
          ...
          media_types = [z.media_devices.MediaType.AUDIO, z.media_devices.MediaType.VIDEO]
          ...
          if key is z.media_devices.MediaType.VIDEO
          ...
          else if key is z.media_devices.MediaType.AUDIO
